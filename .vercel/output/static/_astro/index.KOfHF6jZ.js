...
function s(e, t) {
  if (L((e = e.replace(/#.*/, '')), t?.ignoreSlowConnection ?? !1))
    if (
      (d.add(e),
      document.createElement('link').relList?.supports?.('prefetch') && 'fetch' !== t?.with)
    ) {
      const t = document.createElement('link');
      ((t.rel = 'prefetch'), t.setAttribute('href', e), document.head.append(t));
    } else {
      const t = new Headers();
      for (const [e, n] of Object.entries(v)) t.set(e, n);
      fetch(e, { priority: 'low', headers: t });
    }
}
function L(e, t) {
